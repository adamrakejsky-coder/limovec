...
async def handle_ticket_creation(interaction: discord.Interaction, button_info: dict, ticket_manager):
    ...
    try:
        # Použij ticket manager k vytvoření ticketu (NEPŘEDÁVEJ interaction - my to zpracujeme zde)
        result = await ticket_manager.create_ticket(
            interaction.guild,
            interaction.user,
            button_info['name'],
            button_info['message'],
            None  # Don't pass interaction - we handle response here
        )
        ...
        # Zpracuj response (create_ticket neposlal response, protože jsme předali None)
        if ticket_channel:
            # Success - ticket byl vytvořen
            await interaction.response.send_message(
                f"✅ Ticket vytvořen: {ticket_channel.mention}",
                ephemeral=True
            )
        else:
            # Error - ticket nebyl vytvořen
            await interaction.response.send_message(
                f"❌ {error_msg or 'Nepodařilo se vytvořit ticket.'}",
                ephemeral=True
            )
